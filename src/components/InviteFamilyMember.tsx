...
export function InviteFamilyMember({
  ...
  const updateInvitee = (index: number, field: keyof Invitee, value: string) => {
    setInvitees((prev) =>
      prev.map((invitee, i) =>
        i === index
          ? { ...invitee, [field]: field === "role" ? (value as "viewer" | "contributor") : value }
          : invitee,
      ),
    );
  };
  ...
  const handleSend = async () => {
    ...
    setIsSending(true);
    // Simulate sending invites — will be connected to backend
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="mx-4 w-full max-w-lg rounded-2xl bg-white shadow-2xl">
        ...
        <div className="flex items-center justify-between border-b border-warm-200 px-6 py-4">
          ...
          <button
            className="flex h-8 w-8 items-center justify-center rounded-full text-warm-400 hover:bg-warm-100 hover:text-warm-600"
            onClick={handleClose}
            type="button"
          >
            ✕
        ...
        {sent ? (
          <div className="px-6 py-12 text-center">
            ...
            <p className="mt-2 text-sm text-warm-500">
              ...
              {vaultOwnerName}'s vault.
            </p>
            <button className="btn-primary mt-6" onClick={handleClose} type="button">
              Done
            </button>
          </div>
        ) : (
          <>
            <div className="px-6 py-4">
              <p className="text-sm text-warm-600">
                ...
                <span className="font-semibold text-warm-900">{vaultOwnerName}'s</span>{" "}
            ...
            <div className="space-y-3 px-6 pb-4">
              {invitees.map((invitee, i) => (
                <div key={i} className="flex items-start gap-3">
                  ...
                  <select
                    ...
                    onChange={(e) => updateInvitee(i, "role", e.target.value)}
                    value={invitee.role}
                  >
                  ...
                  {invitees.length > 1 && (
                    <button
                      className="mt-2 shrink-0 text-warm-400 hover:text-red-500"
              ...
              <button
                className="text-sm font-medium text-hearth-600 hover:text-hearth-700"
                onClick={addInvitee}
                type="button"
              >
                + Add another
            ...
            <div className="mx-6 mb-4 space-y-2 rounded-xl bg-warm-50 p-4 text-sm">
              <div className="flex items-start gap-2">
                ...
                <div>
                  <span className="font-medium text-warm-700">Viewer</span>
                  <span className="text-warm-500">
                    {" "}
                    — can see all memories and chat with the Digital Twin
                  </span>
                </div>
              </div>
              <div className="flex items-start gap-2">
                <span className="mt-0.5 text-warm-400">✏️</span>
                <div>
                  <span className="font-medium text-warm-700">Contributor</span>
                  <span className="text-warm-500">
                    {" "}
                    — can also upload new photos, stories, and recordings
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-end gap-3 border-t border-warm-200 px-6 py-4">
              ...
              <button
                ...
                disabled={
                  isSending ||
                  invitees.filter((i) => i.email.trim() && i.email.includes("@"))
                    .length === 0
                }
                onClick={handleSend}
                type="button"
