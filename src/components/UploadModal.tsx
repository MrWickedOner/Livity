...
export function UploadModal({ isOpen, onClose }: UploadModalProps) {
  ...
  const handleUpload = async () => {
    ...
    setIsUploading(true);
    // Simulate upload — will be connected to backend
      ...
      <div className="mx-4 w-full max-w-2xl rounded-2xl bg-white shadow-2xl">
        ...
        <div className="flex gap-1 border-b border-warm-100 px-6 pt-4">
          {uploadTypes.map(({ type, label, icon }) => (
            <button
              key={type}
              className={`flex items-center gap-1.5 rounded-t-xl px-4 py-2 text-sm font-medium transition-all ${
                selectedType === type
          ...
          >
            ...
            {files.length === 0 ? (
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-warm-400">
                    or{" "}
                    <button
                      className="font-medium text-hearth-600 underline hover:text-hearth-700"
              ...
              <div className="space-y-3">
                ...
                <div className="mx-auto max-h-32 space-y-1 overflow-y-auto">
                  {files.map((file, i) => (
                    ...
                    >
                      ...
                      <button
                        className="ml-2 shrink-0 text-warm-400 hover:text-red-500"
                ...
                <button
                  className="text-sm font-medium text-hearth-600 underline hover:text-hearth-700"
                  onClick={() => fileInputRef.current?.click()}
                  type="button"
                >
                  Add more files
        ...
        <div className="flex items-center gap-2 border-t border-warm-100 px-6 py-3">
          ...
          <p className="text-xs text-warm-400">
            Your memories are encrypted end-to-end. Not even Livity can read them.
          </p>
        </div>
